        ...
        <div class="max-w-4xl mx-auto px-4 py-8">
            ...
            <div class="mb-8">
                <h1 class="text-3xl font-bold tracking-tight text-gray-900">
                    <span v-if="!signed">Countersign Contract</span>
                    <span v-else-if="isFullySigned">Contract Fully Executed</span>
                    <span v-else>Your Signature Recorded</span>
                </h1>
                <p class="text-gray-600 mt-2">
                    <span v-if="!signed">
                        Hello {{ coach.name }}, {{ client.name }} has signed the coaching agreement. Please review and
                        provide your countersignature to finalize the contract.
                    </span>
                    <span v-else-if="isFullySigned">
                        This contract has been fully executed by both parties. You can download the final signed copy
                        below.
                    </span>
                    <span v-else>
                        Your countersignature has been recorded. The contract is now fully executed.
                    </span>
                </p>
            </div>
            ...
            <div v-if="!signed" class="bg-white border border-gray-200 rounded-lg shadow-sm">
                <div class="p-6 border-b border-gray-200">
                    <h2 class="text-lg font-semibold text-gray-900">Coach Countersignature Required</h2>
                    <p class="text-sm text-gray-600 mt-1">{{ coach.name }}, please sign in the box below to countersign
                        this
                        coaching contract with {{ client.name }}.</p>
                </div>
                <div class="p-6">
                    <Form :action="store(token).url" method="post" #default="{ processing, errors, recentlySuccessful }"
                        @success="onSuccess">
                        <div class="space-y-4">
                            <!-- Signature Pad -->
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 bg-gray-50">
                                ...
                                <div class="text-xs text-gray-500 mt-3">
                                    By signing, you countersign this agreement and acknowledge its terms and conditions.
                                </div>
                            ...
                            <div v-if="recentlySuccessful" class="text-sm text-green-600">
                                Thank you! Your countersignature has been recorded successfully.
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 pt-6">
                            ...
                            <Button type="submit" :disabled="processing || isEmpty" class="min-w-32">
                                ...
                                {{ processing ? 'Signing...' : 'Countersign Contract' }}
                            </Button>
                        </div>
                    </Form>
                </div>
            ...
            <div v-else class="bg-white border border-gray-200 rounded-lg shadow-sm p-8">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-semibold text-gray-900">Contract Fully Executed</h3>
                            <p class="text-sm text-gray-600">
                                Both you and {{ client.name }} have signed this contract. The agreement is now active.
                            </p>
                        </div>
                    </div>
...
import { signPreview as previewAction, storeSignature as store, pdfToken } from '@/actions/App/Http/Controllers/Tenant/Coach/ContractController'

import { Button } from '@/components/ui/button'
import { CheckCircle, Download, Loader2, PenTool, RotateCcw } from 'lucide-vue-next'

const props = defineProps<{
    token: string;
    contract: { id: number; status: number; is_fully_signed: boolean; client_signed: boolean };
...
const pdfUrl = computed(() => pdfToken(props.token).url)
const signed = computed(() => props.contract?.status >= 4) // Countersigned status
...
const onSuccess = () => {
    console.log('[Coach SignContract] form success - reloading page')
    // After successful sign, reload so server returns updated status and UI shows signed state
