<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <component :is="layoutComponent" v-bind="layoutProps">
            <div class="space-y-6">
                <PageHeader title="Add Task" :description="pageDescription" />
...
const pageDescription = computed(() => {
    return props.session
        ? 'Create a new task for this coaching session'
        : 'Create a new task for this client';
});

const pageTitle = computed(() => {
    return props.session
        ? `Add Task - Session ${props.session.session_number}`
        : `Add Task - ${props.client.name}`;
});
...
const cancel = () => {
    if (props.session) {
        router.visit(sessions.tasks(props.session.id).url);
    } else {
